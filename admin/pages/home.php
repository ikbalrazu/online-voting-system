...
<body>
  ...
  <?php 
             ...
             $id=null;
             $poll = mysqli_query($connect, "Select poll_id,count(poll_id) from vote group by poll_id having count(poll_id)>1");
             while ($p = mysqli_fetch_array($poll))
             echo $p[1]." ";
             $vote_data = mysqli_query($connect, "SELECT * FROM vote");
             while ($vote = mysqli_fetch_array($vote_data)):
